...
배포용 zip을 받은 경우에는 압축을 푼 뒤, 맨 바깥에 있는 `창 뒤 녹화기.exe`만 실행하면 됩니다. 옆에 있는 `창 뒤 녹화기_자료` 폴더는 프로그램이 쓰는 파일들이라 지우지 말아주세요.

개발용 폴더에서 잘 안 열리면 이 파일로 실행해도 됩니다.
...
빌드가 끝나면 같은 폴더에 `창 뒤 녹화기.exe`가 새로 만들어집니다.
...
완성된 zip은 `dist\창 뒤 녹화기.zip`에 만들어집니다. 압축을 풀면 이런 구조입니다.
